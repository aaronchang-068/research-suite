...
配平導數的取得是該方法可行性的關鍵。Raveh 與 Karpel (1999) 建議以線性氣動彈性分析提供近似導數；Raveh、Karpel 與 Yaniv (2000) 在非線性設計載荷的產生流程中進一步證實：導數取自 MSC/NASTRAN 之 Doublet Lattice 模組即已足夠，且導數精度僅影響收斂速率、不影響收斂到的配平解。此一結論大幅降低了演算法的實作門檻，因為它免除了以有限差分對 CFD 求導的昂貴計算。惟兩文之驗證均屬數值模擬層級（L2，Euler CFD 案例），未見風洞或飛試對照——引用其「成本接近單次剛性 CFD」之效率結論時，應限定於同級 CFD 流程內比較。
...
無（已核對 2 份來源——Raveh & Karpel 1999 與 Raveh, Karpel & Yaniv 2000——問題表述、導數處理與耦合強度結論一致；後者為前者之延伸驗證，非獨立歧見）。

## 符號表

符號沿用 Raveh & Karpel (1999) 原文記號，未改記號、未轉軸系（遵循 [[基於空氣動力條件的結構佈局規劃-符號慣例]]）。

| 本筆記符號 | 原文符號（各文獻） | 意義 |
|-----------|-------------------|------|
| $q$, $S$, $\bar c$ | 同左 | 動壓、參考面積、平均氣動弦長 |
| $[M_R]$, $[\phi_R]$ | 同左 | 剛體廣義質量矩陣、剛體模態矩陣 |
| $\{\ddot\xi_R\}$ | 同左 | 指定機動之剛體廣義加速度 |
| $\{F_A\}$ | 同左 | CFD 表面氣動力向量 |
| $\tilde C_{L\alpha},\tilde C_{L\delta},\tilde C_{M\alpha},\tilde C_{M\delta}$ | 同左 | 近似配平導數 |
| 下標 $req$ / $cur$ | 同左 | 目標值／當前值 |
...
## 關鍵要點
- 配平嵌入流場收斂過程逐步修正，撓性機動成本接近剛性 CFD 單次解（限定：L2 數值驗證）。
